...
function HomePerRecommend() {
    ...
    const fetchUserDataAndRecommendations = async (userId) => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          const genres = [
            ...new Set(userGamesData.flatMap((g) => g.game.genres || [])),
          ];
          const themes = [
            ...new Set(userGamesData.flatMap((g) => g.game.themes || [])),
          ];
          ...
          const randomizedRecommended = recommendedGames
            ...
            .slice(0, 8);
          if (isMounted) setGamesData(randomizedRecommended);
        } else {
  ...
  return (
    <div className="home-recommend-container mx-auto px-4 py-6">
      <h1 className="home-trending-head text-3xl font-semibold mb-6 text-gradient">
        RECOMMENDED FOR YOU
      </h1>
