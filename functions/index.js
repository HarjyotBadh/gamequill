...
const TWITCH_CLIENT_ID = "tp9hms3ooc3dxb5n22fp97d73byrhl";
const TWITCH_CLIENT_SECRET = "qhky3fgv8cjyuvh16mhb10jn4d1gqy";

const getAccessToken = async () => {
  const tokenDoc = await db.collection("config").doc("igdb_token").get();
  const now = Date.now();
  ...
  await db.collection("config").doc("igdb_token").set(newToken);
  console.log(
    "New IGDB token obtained, expires in",
  ...
  cors(request, response, async () => {
    ...
    if (request.method === "OPTIONS") {
      ...
      try {
        ...
        response
          .status(500)
          .send({ error: "Failed to fetch release dates from IGDB" });
      }
    }
  });
});
